...
-define(EXPIRATION_TIME, 3000). % time until a message expires, in milliseconds
...
-export([initialize/1,
  % internal actors
  data_actor/1,receive_home_page_stories/3,registerProcess/1]).
...
registerProcess(LastRegisteredId)->
  receive
  {Sender, register_user} ->
    {NewLastRegisteredId, NewUserId} = add_new_user(LastRegisteredId),
    ResponseId = initialize(NewUserId),
    Sender ! {ResponseId, registered_user, NewUserId},
    registerProcess(NewLastRegisteredId)
  end.

add_new_user(LastRegisteredId) ->
  NewUserId = LastRegisteredId,
  % We store the stories in a map id => story.
  % See https://erldocs.com/current/stdlib/maps.html
  NewLastRegisteredId = LastRegisteredId + 1,
  {NewLastRegisteredId, NewUserId}.
